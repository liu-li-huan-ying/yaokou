...
import { fillable, solutionFor } from './solutions'
...
/** 按给定顺序把件装进窑，最多装到本局窑位数 */
function loadFrom(ranked: Pending[], capacity: number): Loading {
  const orderIds: number[] = []
  for (const p of ranked) {
    for (let i = 0; i < p.left && orderIds.length < capacity; i++) orderIds.push(p.orderId)
    if (orderIds.length >= capacity) break
  }
...
/** 只接本局罐里烧得出来的单：接了交不掉是纯亏，不是胆量 */
function acceptFillable(state: RunState): RunState {
  return state.offers.reduce(
    (s, o) => (fillable(o.targetIndex, state.pool) ? accept(s, o.id) : s),
    state,
  )
}
...
const random: Policy = {
  ...
  play: (raw) => {
    ...
    const loading = loadFrom(ranked, state.capacity)
...
const solver: Policy = {
  name: '按单求解',
  play: (raw) => {
    const state = acceptFillable(raw)
    ...
    return fireKiln(state, withSolution(loadFrom(ranked, state.capacity), urgent.targetIndex))
  },
}
...
const adaptive: Policy = {
  ...
  play: (raw) => {
    ...
    for (const o of raw.offers) {
      if (!fillable(o.targetIndex, raw.pool)) continue
      const sol = solutionFor(o.targetIndex)
      const cost = fireCost(sol.recipe, sol.curve)
      const canAfford = state.cash - cost >= ECONOMY.fuelBase
      const feasible = o.deadlineKiln - state.kiln >= Math.ceil(o.qty / state.capacity)
    ...
    return fireKiln(state, withSolution(loadFrom(chosen, state.capacity), best.targetIndex))
  },
}
