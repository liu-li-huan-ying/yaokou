...
export function mountGame(host: HTMLElement, entry: GameEntry): void {
  ...
  function fireSequence(): void {
    ...
    /**
     * rAF 只负责画火，状态推进交给 setTimeout。
     * 挂在 rAF 上会出事：标签页一隐藏浏览器就暂停 rAF，
     * 玩家中途切走这窑就永远烧不完。
     */
    let painting = true
    ...
    window.setTimeout(() => {
      painting = false
      chamber.style.setProperty('--heat', '1')
      mouth.style.setProperty('--heat', '1')
      setFire(0)
      run = next
      clearTray()
      phase = 'result'
  ...
  function revealSequentially(): void {
    ...
    ordered.forEach((p, i) => {
      window.setTimeout(() => {
        ...
        node.btn.classList.add('reveal', 'shown')
        node.btn.dataset.grade = p.grade
        node.grade.textContent = p.grade
        if (p.grade === '珍品') chime()
        else if (p.grade === '废品') crack()
        else knock()
        if (i === ordered.length - 1) window.setTimeout(finishReveal, 340)
      }, i * 260)
    })
  ...
  function renderSlots(): void {
    ...
    slotNodes.forEach((node, i) => {
      ...
      node.btn.classList.remove('reveal', 'shown')
